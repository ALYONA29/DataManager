            ...
            private void Watcher_Created(object sender, FileSystemEventArgs e)
            {
                ...
                if (File.Exists(e.Name))
                {
                    ...
                    if (new FileInfo(e.FullPath).Length != 0)
                    {
                        ...
                        ClassLibrary.Functions.Decrypt(Path.Combine(archive, newPath) + ".xml");
                        File.Move(Path.Combine(archive, newPath + ".xml"), Path.Combine(files, newPath + ".xml"));
